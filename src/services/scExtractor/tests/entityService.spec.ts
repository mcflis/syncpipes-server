...
// test mapper
describe('SocioCortexEntitiesExtractor Test', () => {
    describe('SocioCortexEntitiesExtractor.extract', () => {
        it('should extract all entities', function(done) {
            ...
            service.prepare(context, logger).then(() => {
                service.getConfigSchema(pipeline.extractorConfig).then((config) => {
                    ...
                    // handle end
                    stream.on('end', () => {
                        console.log(this.entities["Kontakt"]);
                        assert.isOk(true, 'extracted entities');
                        done();
                    });
                    // error handling
                    stream.on('error', (err) => {
                        assert.isOk(false, 'could not extract entities');
                        done();
                    });
                });
            });
